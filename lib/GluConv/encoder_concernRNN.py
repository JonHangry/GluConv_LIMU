...
from lib.GluConv.attention import MultiheadAttention
...
class MambaVote(nn.Module):
    ...
    def forward(self, g_data):
        model_dependency = self.forward_mamba(g_data) + self.backward_mamba(g_data.flip(dims=[-1])).flip(dims=[-1])
...
class EncoderConcern(nn.Module):
    ...
    def forward(self, g_data):
        ...
        attention_vote = self.encode_attn(trend_data, trend_data, trend_data)

        to_vote = attention_vote
        all_vote = to_vote
        all_vote = self.activation(all_vote)
        all_vote = self.dropout(all_vote)
