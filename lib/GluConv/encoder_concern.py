...
class EncoderProcessor(nn.Module):
    ...
    def forward(self, g_data):
        ...
        g_data_norm = g_data.permute(0, 2, 1)
        g_data_1 = self.ConvExtender(g_data_norm)
        g_data_2 = self.ConvRestorer(g_data_1)
        g_data_3 = self.ConvRecover(g_data_2)

        g_data_activation = self.activation(g_data_3)

        g_data_processed = self.processor(g_data_activation)

        g_data_end = self.ConvEnder(g_data_processed)
        g_state_end = g_data_end.permute(0,2,1)

        return g_state_end + g_data_res
...
class EncoderConcern(nn.Module):
    ...
    def forward(self, g_data):
        g_data = self.EncoderProcessor(g_data)
        return g_data