...
def objective(trial):
    ...
    writer = SummaryWriter(os.path.join(os.path.dirname(__file__), 
                           f'../output/tensorboard_GluConcern_{args.dataset}/run{trial.number}'))
    model_path = os.path.join(os.path.dirname(__file__),
                              f'../output/tensorboard_GluConcern_{args.dataset}/model.pt')
    ...
    #  TODO 添加自己多出来的部分hd_model
    e_layers = trial.suggest_int("e_layers", 1, 3, step=1)
    d_layers = trial.suggest_int("d_layers", 1, 3, step=1)
    d_model = trial.suggest_int("d_model", 128, 1024, step=32)
    seq_model = trial.suggest_int("seq_model", 128, 1024, step=32)
    r_drop = trial.suggest_float("r_drop", 0.0, 0.1, step=0.01)
    # 1e-5, 2e-5, 3e-5, 4e-5, 5e-5, 6e-5, 7e-5, 8e-5, 9e-5,
    learning_rate = trial.suggest_categorical("learning_rate", [1e-4, 2e-4, 3e-4, 4e-4, 5e-4, 6e-4, 7e-4, 8e-4, 9e-4, 1e-3])
...
if __name__ == '__main__':
    ...
    # load data
    study_file = f'./output/GluConcern_{args.dataset}.txt'
    ...
    r_drop = best_params["r_drop"]
    seq_model = best_params["seq_model"]
    ...
    for model_seed in model_seeds:
        ...
        for seed in seeds:
            writer = SummaryWriter(os.path.join(os.path.dirname(__file__), 
                           f'../output/tensorboard_GluConcern_{args.dataset}/test_run{model_seed}_{seed}'))
            ...
            dataset_test_ood = SamplingDatasetInferenceDual(target_series=series['test_ood']['target'],
                                                            ...
                                                            use_static_covariates=True,
                                                            array_output_only=True)
            # build the NHiTSModel model TODO
            model = GluConcern(
                e_layers = e_layers,
                d_layers = d_layers,
                d_model = d_model,
