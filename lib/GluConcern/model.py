...
class GluConcern(nn.Module):
  def __init__(self, e_layers, d_layers, d_model, len_seq, seq_model, r_drop,
               len_pred, num_dynamic_features, num_static_features, label_len):
    super(GluConcern, self).__init__()
  ...
  def forward(self, x_id, x_enc, x_mark_enc, x_dec, x_mark_dec):
    enc_out = self.enc_embedding(x_id, x_enc, x_mark_enc)  # [B, L, N]
    var_out = self.var(enc_out)

    dec_out = self.SparseAttentionBlock(enc_out)  # [B, L, N]

    dec_out = self.projection(dec_out)  # [B, L, N]
  ...
  def fit(self, 
    ...
    if model_path is not None:
        try:
            ...
            print(f"warning: could not load model at {model_path}\n{e}")
